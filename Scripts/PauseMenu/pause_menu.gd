...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed(Strings.INPUT_ACTION_START) and self.visible and event is not InputEventMouseButton:
		hide_pause_menu()
	elif event.is_action_pressed(Strings.INPUT_ACTION_START) and !self.visible and event is not InputEventMouseButton:
		show_pause_menu()
